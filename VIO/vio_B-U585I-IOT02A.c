...
vioMotionGyro     | vioValueXYZ[0] | iNEMO 3D gyroscope (ISM330DLC)                 |
vioMotionAccelero | vioValueXYZ[1] | iNEMO 3D accelerometer (ISM330DLC)             |
vioMotionMagneto  | vioValueXYZ[2] | High accuracy 3-axis magnetometer (IIS2MDC)    |
*/
...
// Add user variables here:
  ...
  BSP_MOTION_SENSOR_Enable(1U, MOTION_MAGNETO);
#endif
}
...
vioValueXYZ_t vioGetXYZ (uint32_t id) {
  ...
  if (id == vioMotionGyro) {
    if (BSP_MOTION_SENSOR_GetAxes(0U, MOTION_GYRO, &axes) == BSP_ERROR_NONE) {
      vioValueXYZ[index].X = axes.xval;
      vioValueXYZ[index].Y = axes.yval;
      vioValueXYZ[index].Z = axes.zval;
    }
  }

  if (id == vioMotionAccelero) {
    if (BSP_MOTION_SENSOR_GetAxes(0U, MOTION_ACCELERO, &axes) == BSP_ERROR_NONE) {
      vioValueXYZ[index].X = axes.xval;
      vioValueXYZ[index].Y = axes.yval;
      vioValueXYZ[index].Z = axes.zval;
    }
  ...
  if (id == vioMotionMagneto) {
    if (BSP_MOTION_SENSOR_GetAxes(1U, MOTION_MAGNETO, &axes) == BSP_ERROR_NONE) {
      vioValueXYZ[index].X = axes.xval;
      vioValueXYZ[index].Y = axes.yval;
      vioValueXYZ[index].Z = axes.zval;
    }
